...
export function renderAppHostBootstrap(props: AppHostBootstrapProps): string {
  ...
  return [
    ...
    'ENVEOF',
    'fi',
    "cat <<'NGINXEOF' > /opt/ev-dashboard/nginx.ec2-proof.conf",
    proofGatewayConfig,
    'NGINXEOF',
    ...
    'docker run -d --name edge-api-gateway --restart unless-stopped --network ev-dashboard -p 8080:8080 -v /opt/ev-dashboard/nginx.ec2-proof.conf:/etc/nginx/nginx.conf:ro "$GATEWAY_IMAGE"',
    'EOF',
