...
describe('deploy workflow run profiles', () => {
  test('defines full, bootstrap-proof, incremental-expand, warm-host-partial, and smoke-only run profiles', () => {
    ...
    expect(workflowSource).toContain('- incremental-expand');
    expect(workflowSource).toContain('- warm-host-partial');
    expect(workflowSource).toContain('- smoke-only');
  });

  test('runs preflight for full, incremental-expand, and warm-host-partial, but keeps unit tests on full only', () => {
    const workflowPath = path.join(__dirname, '..', '.github', 'workflows', 'deploy-ecs.yml');
    const workflowSource = fs.readFileSync(workflowPath, 'utf8');
  ...
  test('runs synth and deploy for bootstrap-proof and incremental-expand, and smoke for every profile', () => {
    const workflowPath = path.join(__dirname, '..', '.github', 'workflows', 'deploy-ecs.yml');
    const workflowSource = fs.readFileSync(workflowPath, 'utf8');
